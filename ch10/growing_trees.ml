...
let rec size tr =
  match tr with Br (_, l, r) -> 1 + size l + size r | Lf -> 0

let rec total tr =
  match tr with Br (x, l, r) -> x + total l + total r | Lf -> 0
...
let rec list_of_tree tr =
  match tr with
  | Br (x, l, r) ->
      list_of_tree l @ [x] @ list_of_tree r
  | Lf ->
      []
...
let rec tree_map f tr =
  match tr with
  | Br (x, l, r) ->
      Br (f x, tree_map f l, tree_map f r)
  | Lf ->
      Lf
...
(* binary search tree with option *)
let rec lookup2 tr k =
  match tr with
  | Lf ->
      None
  | Br ((k', v), l, r) ->
      if k = k' then Some v
      else if k < k' then lookup2 l k
      else lookup2 r k
